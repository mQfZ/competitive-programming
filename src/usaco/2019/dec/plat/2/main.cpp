...
int main() {
    ...
    y_combinator([&](auto self, int v, int pv) -> void {
        tin[v] = timer++;
        for (int nv : adj[v]) if (nv != pv) self(nv, v);
        tout[v] = timer - 1;
    })(0, -1);
    segtree st(n);
    vector<set<pair<int, int>>> ranges(mx);
    while (q--) {
        int t, x, c; cin >> t >> x, --x;
        if (t == 2) cout << st.query(tin[x], tout[x]).val << '\n';
        else {
            cin >> c;
            for (auto it = ranges[c].lower_bound({tin[x], -1});
                 it != ranges[c].end() && it->first <= tout[x]; ) {
                st.update(it->first, it->second, -1);
                it = ranges[c].erase(it);
            }
            auto it = ranges[c].lower_bound({tin[x], -1});
            bool larger = false;
            if (it != ranges[c].begin()) {
                --it;
                if (it->first <= tin[x] && it->second >= tout[x]) larger = true;
            }
            if (!larger) {
